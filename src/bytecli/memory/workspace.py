...
class WorkspaceMemory(MemoryStore):
    ...
    def _store_path(self) -> Path:
        return self._workspace_dir / ".bytecli" / "workspace_memory.json"
        ...
        try:
            ...
            for item in data.get("entries", []):
                entry = MemoryEntry(
                    ...
                    level=MemoryLevel(item.get("level", "workspace")),
    ...
    async def store(self, entry: MemoryEntry) -> None:
        entry.level = MemoryLevel.WORKSPACE
    ...
    async def compact(self, max_entries: int = 500) -> int:
